...
- Clients can request the current World Tour label at `/v1/assets/composed/featured`. The endpoint selects a ready World Tour job with a raw capture, renders that capture into label `1.png` or `2.png`, writes the passenger name with `BatteryPark.ttf`, and redirects to the rendered image. It uses the newest eligible job from the last 10 minutes, or a random eligible World Tour job if no recent one exists. Override the window with `FEATURED_COMPOSED_RECENCY_SECONDS`; add `?format=json` to inspect the selected job, template, layout version, and stable rendered-image URL.
